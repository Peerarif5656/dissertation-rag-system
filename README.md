...
Note on source documents: This code references a real_documents/ folder containing third-party research PDFs (McKinsey, BCG, Deloitte, IEEE/ICPM papers) that are not redistributed in this repository for copyright reasons. To run the system, supply your own equivalent documents in that folder structure.



Dynamic Processing (pdf_processor.py):

Drag-and-drop functionality: New documents added to a real_documents/ folder are automatically processed and integrated into the RAG system during analysis
...
Similar workflow patterns retrieved via RAG
Labeled scoring data + research papers + user workflow combined
Structured prompt engineering for comprehensive analysis
Response extraction and formatting for interface display
...
Input workflow details (steps, duration, volume)
Select analysis focus area
System performs RAG matching and AI analysis
Results displayed with interactive visualizations
Download comprehensive report with implementation roadmap
...
Note: This repository uses a flat file structure (all files at the top level) rather than nested folders, for ease of browsing. Files are grouped below by function.
...
Note on data: Processed BPI patterns, training/holdout sets, and raw .xes files are not included in this repository due to file size. The raw BPI Challenge 2020 dataset is publicly available from 4TU.ResearchData. Processed JSON outputs can be regenerated using bpi_data_processor.py and the labeling scripts above.
...
bash# Run comprehensive evaluation
python run_comprehensive_evaluation.py

# Test individual components
python -c "from rag_system import BPIRAGSystem; rag = BPIRAGSystem('bpi_rag_data_with_operating_models.json'); print(f'Loaded {len(rag.patterns)} patterns')"
...
Automated Dataset Labeling: Approach using an LLM combined with research literature to label an otherwise unlabeled process mining dataset
RAG-Enhanced Process Analysis: Combines process mining data with a research-paper knowledge base for evidence-grounded recommendations
Cloud-Native Process Optimization: Scalable AWS architecture for enterprise process analysis
Multi-Model Evaluation Framework: Comparison methodology for LLM selection in optimization tasks
